// page/assembleFree/page/AssembleDetail/AssembleDetail.js
import {
  get,
  post,
  wxLogin,
...
Page({
  ...
  // 倒计时
  startTimer: function (totalSecond, v ,type) {
    let that = this
    // 倒计时
    ...
    if (totalSecond == 0) {
      ...
      var interval = setInterval(function () {
        ...
        if(type == 1){
          ...
          this.data.assData[v].countDownHour = hrStr;
          this.data.assData[v].countDownMinute = minStr;
          this.data.assData[v].countDownSecond = secStr;
          this.setData({
            assData: this.data.assData
          });
          totalSecond--;
          if (totalSecond < 0) {
            clearInterval(interval);
            this.data.assData[v].countDownHour = '00';
            this.data.assData[v].countDownMinute = '00';
            this.data.assData[v].countDownSecond = '00';
          }
        }
        // this.data.groupInfo[v].countDownHour = hrStr;
        // this.data.groupInfo[v].countDownMinute = minStr;
  ...
  sure: function (e) {
    ...
    if ((this.data.amountNumber > this.data.inventory) && this.data.channelId != 3) {
      ...
      wx.redirectTo({
        url: '/page/assembleFree/page/groupBalance/groupBalance?id=' + that.data.teamid +
          '&goodid=' + that.data.goodsId + '&skuid=' + that.data.skuid + '&areaid=' + that.data.address.addressId + '&addressIds=' + addressIds + '&goodsnum=' + that.data.amountNumber + '&type=' + that.data.isAssemble + '&found_id=' + that.data.found_id
      })
      that.hideModal()
    }
  },
  ...
  SelectionSpecification: function (e) {
    ...
    if (wx.getStorageSync('uid')){
      ...
      // console.log(itemmid[0])
      // if (itemmid[0].mid == wx.getStorageSync('uid')) {
      //   wx.showToast({
      //     title: '不能参加自己的团',
      //     icon: 'none'
      //   })
      //   return
      // }
      // for (var m = 0; m < itemmid;m++){
      //   if (itemmid[m] >0){
      //     console.log(itemmid[m])
      //   }
      // }
      if (that.data.state != 1) {
        wx.showToast({
  ...
  onLoad: function (options) {
    ...
    console.log(options, options.skuid, options.teamid)
  ...
  detaildata: function () {
    ...
    post('/hd/assembleDetail?assembleId=' + that.data.teamid + '&uid=' + that.data.uid, {}, (res) => {
      wx.hideLoading();
      if (res.data.code == 200) {
        let data = res.data.data
        that.setData({
          banner: data.productInfo.banner,
          team_joiner: data.assembleInfo.team_joiner,
          ...
          success_count: data.assembleInfo.success_count,
          goods_name: data.assembleInfo.goods_name,
          team_rule: data.assembleInfo.team_rule,
        ...
        for (var i = 0; i < this.data.team_joiner.length; i++) {
          this.data.team_joiner[i].pay_time = ((Date.parse(new Date()) - this.data.team_joiner[i].pay_time * 1000)) / 1000 % 60
        }
        this.setData({
          team_joiner: this.data.team_joiner
        })
        // 推荐
        ...
        for (var v = 0; v < data.groupInfo.length; v++) {
          if (data.groupInfo[v].countDown != 0) {
            that.startTimer(data.groupInfo[v].countDown, v,1)
          }
        }
  ...
  getAssdata: function () {
    let that = this
    console.log(that.data.uid)
    post('/hd/getAllAssembleList?assembleId=' + that.data.teamid + '&uid=' + that.data.uid, {}, (res) => {
      wx.hideLoading();
      if (res.data.code == 200) {
        console.log(res)
        let assData = res.data.data
        for (var v = 0; v < assData.length; v++) {
          if (assData[v].countDown != 0) {
            that.startTimer(assData[v].countDown, v,2)
          }
        }
        that.setData({
          assData: assData
